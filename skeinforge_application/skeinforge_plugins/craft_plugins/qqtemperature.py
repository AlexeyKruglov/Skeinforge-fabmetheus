...
def getCraftedTextFromText(gcodeText, repository=None):
	...
	if gcodec.isProcedureDoneOrFileIsEmpty( gcodeText, 'qqtemperature'):
		return gcodeText
	if repository == None:
		repository = settings.getReadRepository( qqTemperatureRepository() )
	if not repository.activateTemperature.value:
		return gcodeText
	return qqTemperatureSkein().getCraftedGcode(gcodeText, repository)

def getNewRepository():
	'Get new repository.'
	return qqTemperatureRepository()

def writeOutput(fileName, shouldAnalyze=True):
	"Temperature a gcode linear move file."
	skeinforge_craft.writeChainTextWithNounMessage(fileName, 'qqtemperature', shouldAnalyze)


class qqTemperatureRepository:
	...
	def __init__(self):
		...
		self.activateTemperature = settings.BooleanSetting().getFromValue('Activate QQTemperature', self, False)
...
class qqTemperatureSkein:
	...
	def getTemperature(self):
		'Get the temperature according to the layer.'
		if self.isBaseLayer:
			return self.baseTemperature
		if self.isInterfaceLayer:
			return self.interfaceTemperature
		if self.isSupportLayer:
			return self.supportTemperature
		if self.layerIndex == 0:
			if self.isEdgePath:
				return self.objectFirstLayerPerimeterTemperature
			else:
				return self.objectFirstLayerInfillTemperature
		return self.objectNextLayersTemperature
	...
	def parseInitialization(self):
		...
		for self.lineIndex in xrange(len(self.lines)):
			...
			if firstWord == '(</extruderInitialization>)':
				self.distanceFeedRate.addTagBracketedProcedure('qqtemperature')
			elif firstWord == '(<threadSequenceString>':
				self.threadSequenceFirstString = splitLine[1]
